...
static struct obj_container *internal_obj_container_alloc(struct obj_container *c, const unsigned int n_buckets, obj_hash_fn *hash_fn, obj_callback_fn *cmp_fn);
...
int internal_obj_ref(void * user_data, const int delta)
{
	struct obj *p = internal_obj(user_data);
	...
	if(current_value <= 0) {
		 ...
		 * allocated. */
		memset(p, '\0', sizeof(struct obj *) + sizeof(void *) );
		spd_safe_free(p);
	}
...
void *internal_obj_alloc(size_t data_size, obj_destructor_fn destructor_fn, const char  *file, const int line, const char *func)
{
	struct obj *p;
...
struct obj_container *__obj_container_alloc(const unsigned int n_bucket, obj_hash_fn *hash_fn, obj_callback_fn *cmp_fn)
{

	const unsigned int num_bucket = hash_fn ? n_bucket : 1;
	
	size_t container_size = sizeof(struct obj_container) + num_bucket *sizeof(struct bucket);
...
{
	struct bucket_entry *p = internal_obj_link(c, user_data, flags, __FILE__, __LINE__, __PRETTY_FUNCTION__);
 ...
 */
static void *internal_obj_callback(struct obj_container *c,const enum search_flags flags, void *cb_fn, void *arg, void *data, enum obj_callback_type type,
				   char *tag, char *file, int line, const char *funcname)
{
	int i, start, last;
	void *ret = NULL;
	 ...
	 for(; i < last; i++) {
		...
		SPD_LIST_TRAVERSE_SAFE_BEGIN(&c->buckets[i], cur, entry) {
			...
			if(flags & OBJ_UNLINK) {
				...
				SPD_LIST_REMOVE_CURRENT(&c->buckets[i],entry)
	...
	if (multi_container != NULL) {
		*multi_iterator = obj_iterator_init(multi_container,OBJ_ITERATOR_DONTLOCK | OBJ_ITERATOR_UNLINK | OBJ_ITERATOR_MALLOCD);
		obj_ref(multi_container, -1);
